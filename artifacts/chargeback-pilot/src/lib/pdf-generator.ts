...
export function generatePdf(data: PdfData): void {
  ...
  doc.text(
    "Keine Rechtsberatung. Keine Erfolgsgarantie. Vorlagen vor Verwendung prüfen.",
    margin,
    25,
  );
  ...
  doc.text(`Score: ${data.successProbability}% — ${data.successProbabilityLabel}`, col2x, y + 15 + lineH * 2);
  ...
  doc.setFont("helvetica", "bold");
  doc.setTextColor(40, 40, 40);
  doc.text(`Erfolgswahrscheinlichkeit: ${data.successProbability}%`, margin, y);
  y += 4;

  doc.setFillColor(220, 230, 240);
  doc.roundedRect(margin, y, usable, 5, 2, 2, "F");

  const pct = Math.min(data.successProbability, 100) / 100;
  const barColor =
    data.successProbability >= 65
      ? [34, 197, 94]
      : data.successProbability >= 40
        ? [234, 179, 8]
        : [239, 68, 68];
  doc.setFillColor(barColor[0], barColor[1], barColor[2]);
  doc.roundedRect(margin, y, usable * pct, 5, 2, 2, "F");
  y += 12;
